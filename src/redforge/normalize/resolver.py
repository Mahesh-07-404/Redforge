        ...
        for h in hosts:
            for t in techs:
                relationships.append(EntityRelation(
                    source_id=h.id,
                    target_id=t.id,
                    relation_type="Host_TO_Technology"
                ))
        ...
        for u in urls:
            for d in dirs:
                relationships.append(EntityRelation(
                    source_id=u.id,
                    target_id=d.id,
                    relation_type="URL_TO_Directory"
                ))
