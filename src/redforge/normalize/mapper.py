...
class HostDiscoveryMapper(BaseMapper):
    def map_output(self, raw_content: str, parsed_content: Dict[str, Any], ref: EvidenceReference, meta: Dict[str, Any]) -> List[NormalizedEntity]:
        entities = []
        hosts = parsed_content.get("subdomains", [])
        if not hosts and raw_content:
            hosts = [line.strip() for line in raw_content.splitlines() if line.strip() and not line.strip().startswith("[")]
            
        for h in hosts:
            entities.append(HostEntity(
                id=f"host_{h}",
                value=h,
...
class HttpxMapper(BaseMapper):
    ...
    def map_output(self, raw_content: str, parsed_content: Dict[str, Any], ref: EvidenceReference, meta: Dict[str, Any]) -> List[NormalizedEntity]:
        ...
        if not urls and raw_content:
            urls = [line.strip() for line in raw_content.splitlines() if line.strip().startswith("http")]
        ...
        for u in urls:
            ...
            entities.append(TechnologyEntity(
    ...
    def map_output(self, raw_content: str, parsed_content: Dict[str, Any], ref: EvidenceReference, meta: Dict[str, Any]) -> List[NormalizedEntity]:
        ...
        for line in raw_content.splitlines():
            ...
            if line_str and not line_str.startswith("["):
                entities.append(DNSRecordEntity(
...
class WebContentMapper(BaseMapper):
    def map_output(self, raw_content: str, parsed_content: Dict[str, Any], ref: EvidenceReference, meta: Dict[str, Any]) -> List[NormalizedEntity]:
        ...
        for line in raw_content.splitlines():
            line_str = line.strip()
            if line_str and not line_str.startswith("#") and ("/" in line_str or "http" in line_str):
