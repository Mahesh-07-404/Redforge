...
class AutonomyController:
    ...
    def __init__(
        ...
        self.consent_timestamp: datetime | None = None
    ...
    def assess_action_risk(self, action_name: str, action_args: dict | None = None) -> ActionRisk:
    ...
    def should_confirm_action(self, action_name: str, action_args: dict | None = None) -> bool:
    ...
    def evaluate_action(
        self, action_name: str, action_description: str = "", action_args: dict | None = None
    ) -> AutonomyDecision:
        ...
        action = Action(
            ...
            can_auto_execute=not requires_confirmation
            and self.current_level != AutonomyLevel.MANUAL,
        ...
        if self.current_level == AutonomyLevel.MANUAL:
            if risk in [ActionRisk.HIGH, ActionRisk.CRITICAL]:
                ...
                return AutonomyDecision(
                    allowed=True,
                    action=action,
                    reason="Action requires confirmation in MANUAL mode",
                    requires_confirmation=True,
            ...
            if risk in [ActionRisk.HIGH, ActionRisk.CRITICAL]:
                return AutonomyDecision(
                    allowed=True,
                    action=action,
                    reason="Action allowed in PARTIAL mode with confirmation",
                    requires_confirmation=True,
    ...
    def confirm_action(self, action_name: str) -> bool:
        ...
        if risk in [ActionRisk.HIGH, ActionRisk.CRITICAL]:
            ...
            if self.destructive_action_count >= self.destructive_threshold:
                self._log_action(
                    "auto_demote", "Auto-demoted to PARTIAL due to destructive actions"
                )
