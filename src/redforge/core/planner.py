...
"""
    ...
    def generate_plan(self, session: SessionState, intent: ParsedIntent) -> List[Dict[str, Any]]:
        ...
        return [
            {"id": "recon", "description": f"Perform initial reconnaissance on {session.target}", "status": "pending"},
            {"id": "scan", "description": f"Scan active services on {session.target}", "status": "pending"},
            {"id": "exploit", "description": "Identify and verify vulnerabilities", "status": "pending"},
