...
from .contracts import ApprovedPlan, ExecutionResult
...
class Executor:
    def __init__(self, stream_manager: StreamManager | None = None):
    ...
    async def execute(self, approved_plan: ApprovedPlan) -> ExecutionResult:
        if approved_plan.decision.status != DecisionStatus.ALLOW:
            raise ValueError(
                f"Cannot execute plan: Policy status is {approved_plan.decision.status.value}"
            )

        start_time = time.time()

        self.stream_manager.emit(
            ExecutionEvent(event_type="ExecutionStarted", data={"goal": approved_plan.plan.goal})
        )
        ...
        self.stream_manager.emit(
            ExecutionEvent(
                event_type="ExecutionFinished", data={"status": status.value, "duration": duration}
            )
        )
