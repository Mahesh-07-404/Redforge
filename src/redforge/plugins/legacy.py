...
class HackerOneAPI(BasePlatform):
    ...
    def get_programs(self) -> List[Program]:
        try:
            response = self.session.get(f"{self.BASE_URL}/programs", params={"filter[state]": "publicly_disclosable"})
    ...
    def submit_report(self, program_handle: str, submission: Submission) -> Dict[str, Any]:
        try:
            ...
            response = self.session.post(f"{self.BASE_URL}/programs/{program_handle}/submissions", json=payload)
            response.raise_for_status()
...
class BugcrowdAPI(BasePlatform):
    ...
    def submit_report(self, program_handle: str, submission: Submission) -> Dict[str, Any]:
        try:
            ...
            response = self.session.post(f"{self.BASE_URL}/submissions", params={"program_handle": program_handle}, json=payload)
            response.raise_for_status()
            return {"success": True, "submission_id": response.json().get("id")}
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to submit report: {e}")
    ...
    def get_reports(self, program_handle: str) -> List[Report]:
        try:
            response = self.session.get(f"{self.BASE_URL}/submissions", params={"program_handle": program_handle})
            response.raise_for_status()
            data = response.json()
            reports = []
            for r in data.get("submissions", []):
...
class PlatformManager:
    ...
    def submit_report(self, program_handle: str, submission: Submission, platform: Platform = None) -> Dict[str, Any]:
        p = platform or self.active_platform
        if not p or p not in self.platforms:
